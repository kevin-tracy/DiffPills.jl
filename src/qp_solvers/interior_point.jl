...
@inline function optnet_cost_terms(z::SVector{n,T},
                                   dz::SVector{n,T}) where {n,T}
#     ∇Qℓ = .5*(dz*z' + z*dz')
#     dℓ_dq = dz
#     dℓ_dQv = vec(∇Qℓ)'
#     return dℓ_dq, dℓ_dQv
    # NOTE: it is totally possible this is incorrect, check qp_solver_tests.jl
    dz', vec(.5*(dz*z' + z*dz'))'
end
@inline function fast_optnet(Q::SMatrix{nx,nx,T,nx2},
                             G::SMatrix{nz,nx,T,nznx},
                             x::SVector{nx,T},
                             s::SVector{nz,T},
                             z::SVector{nz,T},
                             ∇zℓ::SVector{nx,T}) where {nx,nz,T,nx2,nznx}
    # solve for optnet derivatives
    invSZ = Diagonal(z ./ s)
    # # debugging stuff
    # F1 = Symmetric(Q + G'*invSZ*G)
    # if !isposdef(F1)
    #     @show eigvals(F1)
    #     @show F1
    # end
